"""Configuration for VoiceHub-native NeuTTS inference and fine-tuning."""
...
class NeuTTSConfig(VoiceHubConfig):
    """Configure native Llama/Qwen generation and frozen NeuCodec synthesis."""
    ...
    def __init__(
        self,
        *,
        codec_name_or_path: str | Path = str(NEUCODEC_REFERENCE["model_id"]),
        revision: str | None = None,
        codec_revision: str | None = str(NEUCODEC_REFERENCE["revision"]),
    ...
    def validate(self) -> None:
        ...
        if self.sample_rate != 24_000:
            raise ValueError("The released NeuCodec decoder requires `sample_rate=24000`.")
        ...
        self.min_new_tokens = _positive_integer(
            self.min_new_tokens,
            name="min_new_tokens",
        )
        if self.min_new_tokens > self.max_new_tokens:
            raise ValueError("`min_new_tokens` cannot exceed `max_new_tokens`.")
        self.temperature = _finite(self.temperature, name="temperature")
        ...
        if self.trust_remote_code:
            raise ValueError(
                "Native NeuTTS never executes repository code; "
                "`trust_remote_code=True` is unsupported.")
        if self.use_safetensors is False:
            raise ValueError(
                "Native NeuTTS accepts Safetensors only; "
        ...
        for name in ("model_kwargs", "tokenizer_kwargs"):
            ...
            if values:
                ...
                raise ValueError(
                    f"Native NeuTTS does not delegate `{name}`; unsupported "
                    f"option(s): {options}.")
