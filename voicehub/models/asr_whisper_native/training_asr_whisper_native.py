...
class NativeWhisperTrainingAdapter(SpeechSeq2SeqTrainingAdapter):
    ...
    Whisper's native forward pass returns its teacher-forced cross-
    entropy loss directly. The generic sequence objective remains
    available only as a defensive fallback for compatible custom graphs.
    """
    ...
    def setup(self) -> NativeWhisperTrainingAdapter:
        ...
        if getattr(self.model, "architecture_family", None) != "speech-seq2seq":
            raise ValueError("Native Whisper fine-tuning requires the speech-seq2seq "
                             "runtime.")
        if self.primary_model is not getattr(self.model, "model", None):
            raise ValueError("Native Whisper fine-tuning must target the wrapper's exact "
                             "`model` graph.")
        return self
