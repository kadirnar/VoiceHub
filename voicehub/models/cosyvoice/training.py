...
_YAML_IMPORT_PATTERN = re.compile(
    r"(?P<tag>!(?:apply|module|name|new):)"
...
class CosyVoiceTrainingBackend:
    ...
    The public ``model.<component>`` shape intentionally matches the source
    inference wrapper. It lets the shared training adapter use stable component
    paths without constructing the unselected LLM, flow, frontend, or HiFT
    graphs.
    """
...
def _find_model_config(model_directory: Path) -> Path:
    ...
    raise ValueError(
        f"{model_directory} does not contain a CosyVoice model config ({expected})."
    )
...
def _load_component_checkpoint(component, checkpoint_path: Path) -> None:
    ...
    if not isinstance(state_dict, Mapping):
        raise TypeError(
            f"CosyVoice checkpoint {checkpoint_path} must contain a state dictionary."
        )
    model_state = {
        name: value
        for name, value in state_dict.items()
        if name not in _CHECKPOINT_METADATA_KEYS
    }
    try:
...
def load_cosyvoice_training_backend(
    ...
    overrides = {
        name: None
        for name in ("llm", "flow", "hift", "hifigan")
...
class CosyVoiceTrainingAdapter(CompositeTrainingAdapter):
    ...
    CosyVoice's author trainer deliberately selects exactly one of ``llm``,
    ``flow``, or ``hifigan``. VoiceHub mirrors that topology instead of
    pretending these heterogeneous objectives form one model forward.
    """
    ...
    def selected_phase_name(self) -> str:
        selected = (
            str(getattr(self.model.config, "training_component", "llm"))
            .strip()
            .lower()
            .replace("-", "_")
        )
    ...
    def create_optimizer(self, name, parameters, training_args):
        ...
        try:
            ...
            raise ValueError(
                f"Unsupported CosyVoice source optimizer {train_conf['optim']!r}."
            ) from exc
    ...
    def create_scheduler(
        ...
        if normalized_name == "warmuplr":
            ...
            raise ValueError(
                f"Unsupported CosyVoice source scheduler {scheduler_name!r}."
            )
    ...
    def named_parameter_groups(self, training_phase=None):
        selected = (
            self.selected_phase_name if training_phase is None else training_phase
        )
    ...
    def execute_training_phase(
        ...
        try:
            ...
            raise ValueError(
                f"CosyVoice {component_name} has no trainable parameters."
            ) from exc
        output = component(dict(source_batch), device)
        if not isinstance(output, Mapping) or "loss" not in output:
            raise TypeError(
                f"CosyVoice {component_name} must return a mapping containing 'loss'."
            )
        losses = {
            name: value
        ...
        return TTSTrainingOutput(
            ...
            metadata={
                ...
                "metrics": {
                    name: value
                    for name, value in output.items()
                    if name not in losses and name != "logits"
                },
            },
            training_phase=context.phase.name,
            optimizer_names=context.phase.optimizer_names,
    ...
    def save_pretrained(self, save_directory) -> None:
        ...
        component_name = (
            "llm" if self.selected_phase_name == "language_model" else "flow"
        )
        component = getattr(self.model.model.model, component_name)
        state = {
