...
REFERENCE_XCODEC2_CHECKPOINT = MappingProxyType(
    {
        ...
        "revision": "64bd034d12d441299cdd535b15c33efd6ccdf252",
        "filename": "model.safetensors",
        "sha256": (
            "611a63e4dff70c19bd4718d701bb7bc522acf6293a109ab62f5db2f7ff395114"
        ),
        "size": 2_517_231_448,
        "tensor_count": 811,
    }
...
class XCodec2CheckpointAdapter(CheckpointAdapter):
    ...
    def __init__(self, tensor_names: tuple[str, ...]) -> None:
        if (
            not isinstance(tensor_names, tuple)
            or not tensor_names
    ...
    def probe(
        ...
        return (
            str(config.get("model_type", "")).lower() == "xcodec2"
            and any(path.suffix == ".safetensors" for path in files)
        )

    def tensor_plan(self, config: Mapping[str, Any]) -> TensorPlan:
        del config
        return TensorPlan(
            rules=tuple(CopyTensor(name, name) for name in self.tensor_names)
        )
