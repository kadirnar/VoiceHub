...
The public LLaSA checkpoints extend the Llama-3 tokenizer with eight modality
markers followed by one added token for each of XCodec2's 65,536 codes.  This
module reads the declarative ``tokenizer.json`` asset directly; it never
imports or executes tokenizer provider code.
"""
...
LLASA_SPEECH_TOKEN_OFFSET = (
    LLASA_BASE_VOCABULARY_SIZE + LLASA_MODALITY_TOKEN_COUNT
)
LLASA_VOCABULARY_SIZE = (
    LLASA_SPEECH_TOKEN_OFFSET + LLASA_SPEECH_CODEBOOK_SIZE
)
...
def _tokenizer_document(path: Path) -> dict[str, Any]:
    try:
        ...
        raise TokenizerAssetError(
            f"Invalid LLaSA tokenizer JSON: {error}."
        ) from error
...
class LlasaTokenizer:
    ...
    def from_tokenizer_json(
        ...
        *,
        ...
        config_path = (
            ...
            else Path(tokenizer_config_path).expanduser().resolve()
        )
        return cls(
            tokenizer,
    ...
    def _validate_messages(
        ...
        for index, message in enumerate(messages):
            ...
            if role not in {"system", "user", "assistant"}:
                raise ValueError(
                    f"LLaSA message {index} has unsupported role {role!r}."
                )
            if not isinstance(content, str):
                raise TypeError(
                    f"LLaSA message {index} content must be a string."
                )
    ...
    def format_chat(
        ...
        *,
        ...
        """Render the checkpoint's Llama-3.2 chat template deterministically."""
        ...
        if continue_final_message and normalized[-1]["role"] != "assistant":
            raise ValueError(
                "LLaSA continuation requires a final assistant message."
            )
