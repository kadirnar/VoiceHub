...
class StyleTTS2Runtime:
    ...
    def __init__(
        ...
        *,
        ...
        if missing:
            raise FileNotFoundError("Missing StyleTTS 2 checkpoint assets: " + ", ".join(missing))
    ...
    def _length_to_mask(self, lengths):
        torch = self._torch
        positions = (
            torch.arange(lengths.max(), device=lengths.device).unsqueeze(0).expand(lengths.shape[0], -1))
    ...
    def generate(
        ...
        *,
        ...
        return (audio[..., :-50] if self.model_params.decoder.type == "hifigan" else audio)
