    ...
    def __init__(
        ...
        *,
        ...
        for index, record in enumerate(records):
            ...
            value = dict(record)
            if not any(
                name in value for name in ("audio", "audio_path", "input_values")
            ):
                raise ValueError(f"MarbleNet VAD record {index} requires raw audio.")
            if not any(
                name in value for name in ("labels", "frame_labels", "segments")
            ):
                raise ValueError(
                    f"MarbleNet VAD record {index} requires frame or segment targets."
                )
    ...
    *,
    ...
    if isinstance(value, (tuple, list)):
        ...
        if (
            ...
            and batch_size == 1
            and len(value) == 1
            and isinstance(value[0], (tuple, list, Mapping))
        ):
            return value[0]
    return value
...
def _labels_from_segments(
    ...
    *,
    ...
    if isinstance(segments, Mapping) or (
        isinstance(segments, Sequence)
        and not isinstance(segments, (str, bytes))
        and len(segments) == 2
        and all(isinstance(item, Real) and not isinstance(item, bool) for item in segments)
    ):
        segments = [segments]
    if (
        isinstance(segments, (str, bytes))
        or not isinstance(segments, Iterable)
    ):
...
def _explicit_labels(value: Any, *, frame_count: int):
    ...
    if labels.numel() != frame_count:
        raise ValueError(
            f"MarbleNet expected {frame_count} frame labels, "
...
def _augment_waveform(
    ...
    *,
    ...
    if (
        noise_source is not None
        and torch.rand((), device=values.device) < config.training_noise_probability
    ):
        ...
        ).waveform.to(device=values.device, dtype=values.dtype)
        if noise.numel() == 0:
...
def prepare_marblenet_vad_training_batch(
    ...
    source_name = next(
        (
            name
            for name in ("waveforms", "input_values", "audio", "audio_path")
            if name in inputs
        ),
        None,
    )
    if source_name is None:
        raise ValueError(
            "MarbleNet training requires `audio`, `audio_path`, or `input_values`."
        )
    ...
    if explicit is None and segment_values is None:
        raise ValueError(
            "MarbleNet training requires aligned frame labels or speech segments."
        )
    ...
    waveform_batch = torch.stack(
        [
            functional.pad(value, (0, max_samples - value.numel()))
...
class NativeMarbleNetVADTrainingAdapter(FrameClassificationTrainingAdapter):
    ...
    def setup(self):
        ...
        if self.primary_model is not getattr(self.model, "model", None):
            raise ValueError(
                "Native MarbleNet tuning must target the wrapper's exact model."
            )
        return self
    ...
    def create_optimizer(
        ...
        if name not in {"default", "model"}:
            raise ValueError(
                "MarbleNet VAD declares only the `model` optimizer, "
                f"found {name!r}."
            )
        values = [
            parameter
    ...
    def create_scheduler(
        ...
        if name not in {"default", "model"}:
            raise ValueError(
                "MarbleNet VAD declares only the `model` scheduler, "
                f"found {name!r}."
            )
