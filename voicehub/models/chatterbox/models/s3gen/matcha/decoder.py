...
def get_activation(name: str) -> nn.Module:
    """Return the activation names used by the released decoder."""
    ...
    try:
        ...
        raise ValueError(f"Unsupported Chatterbox activation: {name!r}") from error
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(channels)):  # pylint: disable=consider-using-enumerate
            ...
            downsample = (
                Downsample1D(output_channel) if not is_last else nn.Conv1d(
        ...
        for i in range(len(channels) - 1):
            ...
            upsample = (
                Upsample1D(output_channel, use_conv_transpose=True) if not is_last else nn.Conv1d(
                    output_channel, output_channel, 3, padding=1))
