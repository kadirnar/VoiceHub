...
class Model(nn.Module):
    """Invertible WavMark encoder/decoder implemented with native PyTorch."""
    ...
    def istft(self, signal_wmd_fft):
        window = torch.hann_window(
            self.n_fft,
            device=signal_wmd_fft.device,
            dtype=signal_wmd_fft.dtype,
        )
        return torch.istft(
            torch.view_as_complex(signal_wmd_fft.contiguous()),
            n_fft=self.n_fft,
            hop_length=self.hop_length,
            window=window,
