...
def apply_delay_pattern(codes: Tensor, mask_token_id: int) -> Tensor:
    ...
    if not isinstance(codes, Tensor) or codes.ndim != 3:
        raise ValueError(
            "Zonos codes must have shape [batch, codebook, time]."
        )
    if codes.shape[1] == 0:
    ...
    return torch.stack(
        [
            padded[:, index].roll(index + 1, dims=-1)
            for index in range(padded.shape[1])
        ],
        dim=1,
    )
...
def revert_delay_pattern(codes: Tensor) -> Tensor:
    ...
    if not isinstance(codes, Tensor) or codes.ndim != 3:
        raise ValueError(
            "Delayed Zonos codes must have shape [batch, codebook, time]."
        )
    _, num_codebooks, sequence_length = codes.shape
    if sequence_length < num_codebooks + 1:
        raise ValueError(
            "Delayed Zonos sequence is too short to contain one audio frame."
        )
    return torch.stack(
        [
            codes[
...
def add_endpoint_and_delay(
    ...
    *,
    ...
    if not isinstance(audio_codes, Tensor) or audio_codes.ndim != 3:
        raise ValueError(
            "Zonos audio codes must have shape [batch, codebook, time]."
        )
    ...
    if lengths is None:
        ...
        raise ValueError(
            "Zonos audio-code lengths must have shape [batch]."
        )
    else:
        lengths = lengths.to(
            device=audio_codes.device,
    ...
    if bool(((active < 0) | (active >= eos_token_id)).any()):
        raise ValueError(
            f"Valid Zonos codec tokens must be in [0, {eos_token_id - 1}]."
        )
    endpoint = torch.full(
