...
class ESPnetLibriSpeechTransformerConfig:
    ...
    ``librispeech-transformer-e18`` is deliberately immutable: accepting an
    arbitrary ESPnet YAML under the same architecture ID would turn strict
    checkpoint validation into guesswork.  Small tests and research graphs
    can use ``variant="custom"``.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.f_min < self.f_max <= self.sampling_rate / 2:
            raise ValueError(
                "Mel bounds must satisfy 0 <= f_min < f_max <= Nyquist."
            )
        if self.maximum_decode_ratio <= self.minimum_decode_ratio:
            raise ValueError(
                "`maximum_decode_ratio` must exceed `minimum_decode_ratio`."
            )
        for name in (
            "blank_token_id",
            "unknown_token_id",
        ...
        if (
            isinstance(self.ignore_token_id, bool)
            or not isinstance(self.ignore_token_id, int)
        ):
    ...
    def from_dict(
        ...
        extra = {
            ...
            if key not in allowed and key not in ignored
        }
        for key in ignored:
            source.pop(key, None)
