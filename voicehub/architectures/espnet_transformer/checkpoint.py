    ...
    *,
    ...
    rows = [
        (
            ...
            f"{'x'.join(str(item) for item in tensor.shape)}"
        )
...
def native_espnet_asr_tensor_shapes(
...
def native_espnet_lm_tensor_shapes(
    config: _ConfigLike = None,
) -> dict[str, tuple[int, ...]]:
    import torch
...
def _verify_file(
    ...
    *,
    ...
    if expected_sha256 is not None and digest != expected_sha256:
        raise ValueError(
            f"ESPnet {label} SHA-256 mismatch: expected "
...
def _restricted_state_dict(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"Could not read the restricted ESPnet {label} tensor state."
        ) from error
    ...
    if any(
        ...
        raise TypeError(
            f"ESPnet {label} must map string names to tensors only."
        )
    return payload
    ...
    for line in lines:
        ...
        if scalar.startswith('"'):
            try:
                ...
                raise ValueError(
                    "ESPnet token list contains an invalid quoted scalar."
                ) from error
    ...
    *,
    ...
    if official:
        ...
        if source_fingerprint != ESPNET_LM_SOURCE_TENSOR_FINGERPRINT:
            raise ValueError(
                "ESPnet language-model tensor inventory fingerprint mismatch."
            )
    if any(not name.startswith("lm.") for name in state):
        raise ValueError(
            "ESPnet language-model tensors must use the `lm.` source prefix."
        )
    native = {
        name[3:]: tensor
...
def convert_espnet_librispeech_checkpoints(
    *,
    ...
    if official:
        ...
        conflicts = [
            name
            for name, (supplied, pinned) in supplied_expectations.items()
            if supplied is not None and supplied != pinned
        ]
        if conflicts:
            raise ValueError(
                "Official ESPnet conversion expectations are immutable: "
                + ", ".join(conflicts)
                + "."
            )
    ...
    if official and (
        len(token_payload) != ESPNET_TOKEN_LIST_SIZE
        or hashlib.sha256(token_payload).hexdigest() != ESPNET_TOKEN_LIST_SHA256
    ):
    ...
    with tempfile.TemporaryDirectory(
        ...
        values.update({
            ...
            "checkpoint_format": NATIVE_ESPNET_FORMAT,
            "model_type": "asr_espnet",
            "source_artifact_revision": ESPNET_REVISION,
            "source_asr_sha256": digests["asr"],
            "source_asr_tensor_fingerprint": asr_fingerprint,
            "source_config_sha256": digests["config"],
            "source_lm_sha256": digests["lm"],
            "source_lm_tensor_fingerprint": lm_fingerprint,
            "source_revision": ESPNET_SOURCE_REVISION,
            "source_tokenizer_sha256": digests["tokenizer"],
            "voicehub_provider": "asr_espnet",
        })
...
def _validate_safe_inventory(
    ...
    *,
    ...
    with SafeTensorReader(path) as reader:
        ...
        if mismatches:
            raise ValueError(
                f"ESPnet native {label} tensor shape mismatch: {mismatches}."
            )
    ...
    *,
    ...
    with SafeTensorReader(asr_path) as reader:
        ...
        if declared != NATIVE_ESPNET_FORMAT:
            raise ValueError(
                f"ESPnet Safetensors declares unsupported format {declared!r}."
            )
