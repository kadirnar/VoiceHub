...
"""Native PyTorch MOSS Audio Tokenizer v1 model."""
...
def auto_docstring(*args, **kwargs):
    """Compatibility no-op for upstream documentation decorators."""
    del kwargs
...
class MossAudioTokenizerTransformerLayer(StreamingModule):
    ...
    def __init__(
        ...
        if weights_per_step:
            num_weights = max(
                weights_per_step_schedule) + 1 if weights_per_step_schedule else weights_per_step
        ...
        if gating == "none":
            ...
            if weights_per_step:
                dim_ff_list = [dim_feedforward] * num_weights if isinstance(
                    dim_feedforward, int) else dim_feedforward
...
class MossAudioTokenizerPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if not isinstance(loaded.model, cls):
            raise ValueError("The checkpoint does not contain a MOSS codec v1 graph.")
        return loaded.model
...
class MossAudioTokenizerModel(MossAudioTokenizerPreTrainedModel):
    ...
    def batch_encode(
            ...
            num_quantizers: int | None = None) -> MossAudioTokenizerEncoderOutput:
    ...
    def batch_decode(
            ...
            num_quantizers: int | None = None) -> MossAudioTokenizerDecoderOutput:
    ...
    def forward(
        ...
        ```python
        >>> import torch
        >>> from voicehub.architectures.mosstts import MossAudioTokenizerV1Model

        >>> model = MossAudioTokenizerV1Model.from_pretrained(
...
MossAudioTokenizerV1Model = MossAudioTokenizerModel
