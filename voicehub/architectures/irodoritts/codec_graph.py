...
class Decoder(nn.Module):
    ...
    def watermark(self, x, message: Optional[torch.Tensor] = None):
        if self.alpha == 0.0:
            # The published Semantic-DACVAE checkpoint was used by Irodori
            # with watermark insertion disabled, while retaining the
            # encoder block's final mono projection.
