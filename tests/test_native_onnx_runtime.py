...
class NativeONNXRuntimeTests(unittest.TestCase):
    ...
    def test_state_loading_is_shape_checked_and_supports_partial_updates(self):
        ...
        missing, unexpected = runtime.load_onnx_state_dict(
    ...
    def test_semantic_fingerprint_ignores_weights_but_covers_attributes(self):
        ...
        changed_weight = onnx_semantic_fingerprint(
            graph(weight=2.0, axis=-1)
        )
        changed_attribute = onnx_semantic_fingerprint(
            graph(weight=1.0, axis=0)
        )
    ...
    def test_dependency_free_reader_preserves_constant_tensor_attributes(self):
        tensor = (
            _varint_field(1, 2)
            + _varint_field(2, 1)
            + _bytes_field(9, struct.pack("<2f", 1.25, -2.5))
        )
        attribute = (
            _bytes_field(1, b"value")
            + _varint_field(20, 4)
            + _bytes_field(5, tensor)
        )
        node = (
            _bytes_field(2, b"y")
            + _bytes_field(4, b"Constant")
            + _bytes_field(5, attribute)
        )
        output = _bytes_field(1, b"y")
        graph = (
            _bytes_field(1, node)
            + _bytes_field(2, b"constant-test")
            + _bytes_field(12, output)
        )
        opset = _varint_field(2, 19)
        model_bytes = (
            _varint_field(1, 9)
            + _bytes_field(7, graph)
            + _bytes_field(8, opset)
        )
