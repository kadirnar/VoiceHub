...
VoiceHub는 공통 설정, 프로세서, 모델, 생성 출력, 트레이너 API를 통해 텍스트
음성 변환 통합 기능을 제공합니다. 모델 구현은 각 아키텍처의 특성을 그대로
유지합니다. 코덱 언어 모델, 시퀀스-투-시퀀스 시스템, 플로 매칭 및 확산 모델,
음향 모델, VITS 스타일 적대적 시스템, 복합 파이프라인은 각자의 조건화 방식,
목적 함수, 파라미터 소유권, 내보내기 규칙을 유지합니다.

레지스트리에는 **31개의 추론 통합**이 있습니다. 이 중 **18개에는 문서화된 미세
조정 경로**가 있으며, **6개는 일반 원시 레코드를 직접 입력받을 수 있습니다**.
미세 조정 지원 여부는 checkpoint와 런타임에 따라 달라집니다. 추론을 지원한다고
해서 현재 VoiceHub 아티팩트가 미분 가능한 것은 아닙니다. 통합 기능을 선택할 때는
[모델 카탈로그](models/index.md)와
[checkpoint별 학습 매트릭스](models/training-support.md)를 확인하세요.
