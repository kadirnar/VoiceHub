...
eight-section detail contract to every registered model without importing its
implementation. It resolves and links the stable configuration and model
facades, requires at least four parseable Python examples, separates checkpoint
defaults from execution evidence, and states optional-dependency, hardware,
optimization-failure, and real-checkpoint limitations explicitly.
...
The focused regression failed before implementation because Auto Classes and
the 68 generated guides still lived under `Base classes → Models`. The shared
generator now writes Auto Classes exactly once under `API → Main Classes` and
all 68 unique model paths under `API → Models`, grouped as 34 TTS, 23 ASR, and
11 VAD guides. Base classes retains lifecycle and support material,
contribution, preprocessors, and architecture. Auto Classes now expands `API →
Main Classes`; SpeechT5 expands `API → Models → Text to speech`. Model routes
and page content remain unchanged, footer adjacency follows the new document
order, and the generator is idempotent.
...
signatures, 60 complete focus cycles with 4,575 stops, 30 expanded-state Axe
cases, 32 root-branch activation/restoration cases, two mobile drawer cases,
40 source navigations, 40 theme switches, 40 language switches, 60 version
cases, 60 search cases, and 40 TOC cases. A final 12-case model-route slice
revalidated exact API ancestry, screenshots, Axe, and 1,970 focus stops after
formatting and the final strict build. The keyboard inventory remains 300
cases.
...
representative route. Its six visible nested disclosure paths are `API → Main
Classes`, `API → Models`, `API → Models → Text to speech`, `API → Models → Text
to speech → SpeechT5`, `API → Models → Automatic speech recognition`, and `API
→ Models → Voice activity detection`. Each must activate and return to its
...
palettes. Twelve default-palette cases use Enter and 12 slate cases use a
