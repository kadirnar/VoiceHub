...
يقدّم VoiceHub تكاملات تحويل النص إلى كلام من خلال واجهات API مشتركة
للتكوين والمعالج والنموذج ومخرجات التوليد والمدرّب. وتظل تطبيقات النماذج
...
يضم registry **31 تكاملاً للاستدلال**. ولدى **18 منها مسار موثّق للضبط
الدقيق**، منها **6 تقبل السجلات الأولية العادية**. يعتمد دعم الضبط الدقيق
على نقطة التحقق وبيئة التشغيل تحديدًا؛ فوجود تكامل للاستدلال لا يعني أن
artifact الحالي الخاص به في VoiceHub قابل للاشتقاق. استخدم
[دليل النماذج](models/index.md) و
[مصفوفة التدريب المراعية لنقاط التحقق](models/training-support.md)
لاختيار التكامل المناسب.

تُضمّن الشيفرة المصدرية للنموذج مع VoiceHub. تثبّت extras الاختيارية
اعتماديات بيئة التشغيل المحددة، بينما تُنزّل أوزان نقاط التحقق عند الحاجة
أو تُوفّر عبر مسارات محلية. لا يغطي ترخيص Apache-2.0 سوى VoiceHub نفسه؛
وقد تخضع الشيفرة المصدرية المدمجة ونقاط التحقق وبرامج codec ومجموعات
البيانات والصوت المُنشأ لشروط منفصلة.
...
-   **النماذج**
    ...
    قارن جميع إدخالات registry البالغ عددها 31، وextras التثبيت، ونقاط
    التحقق الافتراضية، والقدرات، ومصدر الشيفرة، والقيود.
...
-   **Notebook**
    ...
    نفّذ سير عمل Dia بدءًا من الاستدلال الأساسي والتحقق من صحة البيانات،
    مرورًا بالتدريب والتصدير، وانتهاءً بإعادة التحميل في بيئة تشغيل جديدة.

    [افتح دليل Notebook](guides/notebook.md)
