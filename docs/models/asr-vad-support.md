---
description: Compare VoiceHub ASR and VAD providers, architecture families, the consolidated install bundle, outputs, and fine-tuning boundaries.
---
...
| --- | --- | --- | --- | --- |
| `asr_transformers` | Transformers CTC, speech seq2seq, RNN-T, and TDT auto-models; covers compatible Whisper, Wav2Vec2/HuBERT/WavLM/MMS, Speech2Text, Parakeet, and future registered architectures | Text, segments, word/segment timestamps when emitted, language | `asr-vad` | **VoiceHub native** for a compatible unquantized differentiable checkpoint |
| `asr_faster_whisper` | Whisper through CTranslate2 | Multilingual text and timestamps | `asr-vad` | **Inference-only**; fine-tune the Transformers checkpoint before conversion |
| `asr_whisperx` | WhisperX transcription and optional alignment | Word alignment and speaker fields when emitted | `asr-vad` | **Inference-only**; fine-tune the underlying Whisper model separately |
| `asr_openai_whisper` | OpenAI Whisper reference runtime | Multilingual text and timestamps | `asr-vad` | **Inference-only** in VoiceHub; use `asr_transformers` for fine-tuning |
| `asr_nemo` | NeMo ASRModel families, including compatible Canary and Parakeet checkpoints | Provider text and timestamps when emitted; VoiceHub's current session API is buffered/offline | `asr-vad` | **Upstream-custom** NeMo Lightning/Hydra recipe |
| `asr_speechbrain` | SpeechBrain encoder-decoder/CTC recipes such as CRDNN | Text and provider metadata | `asr-vad` | **Upstream-custom** Brain/hparams recipe |
| `asr_funasr` | FunASR families such as Paraformer and SenseVoice | Multilingual text, timestamps, punctuation/VAD/speaker composition when configured | `asr-vad` | **Upstream-custom** FunASR task/configuration runner |
| `asr_espnet` | ESPnet Speech2Text and model-zoo ASR artifacts | Text, hypotheses, scores when emitted | `asr-vad` | **Upstream-custom** ESPnet ASRTask recipe |
| `asr_wenet` | WeNet ASR runtime, including upstream streaming-capable families | Text and provider metadata; VoiceHub's current session API is buffered/offline | `asr-vad` | **Upstream-custom** WeNet recipe |
...
| Model type | Provider and family coverage | Normalized capability | Install extra | Fine-tuning boundary |
| --- | --- | --- | --- | --- |
| `vad_transformers` | Compatible Transformers audio- and frame-classification checkpoints | Speech regions plus real frame/window probabilities when requested | `asr-vad` | **VoiceHub native** classification path for differentiable checkpoints |
| `vad_silero` | Official Silero JIT or ONNX VAD runtime | Speech timestamps; 8/16 kHz; buffered streaming contract | `asr-vad` | **Inference-only**; the published package does not expose its training recipe |
| `vad_webrtc` | WebRTC fixed-point GMM | Binary frame decisions normalized to regions; 8/16/32/48 kHz | `asr-vad` | **Inference-only** fixed algorithm |
| `vad_pyannote` | pyannote.audio voice-activity pipeline/segmentation artifacts | Segmentation regions and scores when emitted | `asr-vad` | **Upstream-custom** pyannote task/data/trainer recipe |
| `vad_speechbrain` | SpeechBrain CRDNN VAD | Native chunk post-processing normalized to regions | `asr-vad` | **Upstream-custom** SpeechBrain recipe |
| `vad_nemo` | NeMo MarbleNet window/frame VAD | Frame/window probabilities normalized with common post-processing | `asr-vad` | **Upstream-custom** NeMo data/configuration recipe |
| `vad_funasr` | FunASR FSMN VAD through ModelScope or Hugging Face artifacts | 16 kHz speech boundaries; native milliseconds normalized to public seconds | `asr-vad` | **Upstream-custom** FunASR configuration-driven training runner |
...
`asr-vad` is the only public ASR/VAD inference extra. Every speech-input
`ModelSpec.install_extra` and missing-dependency message points to that bundle.
