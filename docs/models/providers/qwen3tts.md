...
Qwen3TTS uses the canonical model type `qwen3tts` and is a
VoiceHub **text to speech** integration. This page is
generated from the model registry and its executable data and training
contracts, so the documented support stays aligned with code. [Open the `qwen3tts` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/qwen3tts.ipynb).
...
The loader returns `Qwen3TTSForTextToSpeech` through the shared task-specific factory.
