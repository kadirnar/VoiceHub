...
PyannoteSegmentationVAD uses the canonical model type `vad_pyannote_segmentation` and is a
...
contracts, so the documented support stays aligned with code. [Open the `vad_pyannote_segmentation` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/vad_pyannote_segmentation.ipynb).
...
The loader returns `PyannoteSegmentationVADForVoiceActivityDetection` through the shared task-specific factory.
