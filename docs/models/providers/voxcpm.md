...
VoxCPM uses the canonical model type `voxcpm` and is a
VoiceHub **text to speech** integration. This page is
generated from the model registry and its executable data and training
contracts, so the documented support stays aligned with code. [Open the `voxcpm` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/voxcpm.ipynb).
...
The loader returns `VoxCPMForTextToSpeech` through the shared task-specific factory.
