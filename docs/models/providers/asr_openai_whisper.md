---
description: Inference, data preparation, and training guide for the asr_openai_whisper integration.
---

# `asr_openai_whisper` model guide

`asr_openai_whisper` is a VoiceHub **automatic speech recognition**
integration. This page is generated from the model registry and its executable
data and training contracts, so the documented support stays aligned with code. [Open the `asr_openai_whisper` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_openai_whisper.ipynb).
...
The `asr_openai_whisper` contract is **integrated-raw**. Its
...
contract = get_asr_dataset_spec('asr_openai_whisper')
...
arguments = TrainingArguments(
    output_dir="runs/asr_openai_whisper-smoke",
    max_steps=1,
...
trainer.save_model("runs/asr_openai_whisper-smoke/final")
```
