...
OpenAIWhisper uses the canonical model type `asr_openai_whisper` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_openai_whisper` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_openai_whisper.ipynb).
...
OpenAI Whisper-compatible records trained through the native Whisper graph. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `OpenAIWhisperForSpeechRecognition` through the shared task-specific factory.
