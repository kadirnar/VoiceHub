...
Tiron uses the canonical model type `asr_tiron` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_tiron` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_tiron.ipynb).
...
The loader returns `TironForSpeechRecognition` through the shared task-specific factory.
