...
FunASR uses the canonical model type `asr_funasr` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_funasr` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_funasr.ipynb).
...
The loader returns `FunASRForSpeechRecognition` through the shared task-specific factory.
