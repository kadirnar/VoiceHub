...
FasterWhisper uses the canonical model type `asr_faster_whisper` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_faster_whisper` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_faster_whisper.ipynb).
...
Faster-Whisper-compatible records trained through the native Whisper graph. Follow the [shared data workflow](../../guides/speech-data.md) for
...
[View `FasterWhisperConfig` source](https://github.com/kadirnar/voicehub/blob/main/voicehub/models/asr_native/configuration.py)
...
[View `FasterWhisperForSpeechRecognition` source](https://github.com/kadirnar/voicehub/blob/main/voicehub/models/asr_native/faster_whisper.py)
...
The loader returns `FasterWhisperForSpeechRecognition` through the shared task-specific factory.
