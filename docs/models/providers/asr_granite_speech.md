...
GraniteSpeech uses the canonical model type `asr_granite_speech` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_granite_speech` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_granite_speech.ipynb).
...
Prompt-conditioned Granite Speech multimodal fine-tuning records. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `GraniteSpeechForSpeechRecognition` through the shared task-specific factory.
