...
MedASR uses the canonical model type `asr_medasr` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_medasr` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_medasr.ipynb).
...
The loader returns `MedASRForSpeechRecognition` through the shared task-specific factory.
