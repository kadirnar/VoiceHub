...
Moonshine uses the canonical model type `asr_moonshine` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_moonshine` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_moonshine.ipynb).
...
The loader returns `MoonshineForSpeechRecognition` through the shared task-specific factory.
