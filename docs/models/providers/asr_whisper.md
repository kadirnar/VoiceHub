...
Whisper uses the canonical model type `asr_whisper` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_whisper` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_whisper.ipynb).
...
Native Whisper transcription or translation fine-tuning records. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `WhisperForSpeechRecognition` through the shared task-specific factory.
