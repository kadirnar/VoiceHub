...
TransformersASR uses the canonical model type `asr_transformers` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_transformers` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_transformers.ipynb).
...
Checkpoint-dispatched raw and cached inputs for native Transformers ASR families. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `TransformersASRForSpeechRecognition` through the shared task-specific factory.
