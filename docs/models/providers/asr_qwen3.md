...
Qwen3ASR uses the canonical model type `asr_qwen3` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_qwen3` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_qwen3.ipynb).
...
Qwen3-ASR completion-only multimodal fine-tuning records. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `Qwen3ASRForSpeechRecognition` through the shared task-specific factory.
