...
WhisperX uses the canonical model type `asr_whisperx` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_whisperx` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_whisperx.ipynb).
...
WhisperX-compatible records trained through the native Whisper graph. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `WhisperXForSpeechRecognition` through the shared task-specific factory.
