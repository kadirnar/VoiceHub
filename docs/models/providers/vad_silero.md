...
SileroVAD uses the canonical model type `vad_silero` and is a
...
contracts, so the documented support stays aligned with code. [Open the `vad_silero` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/vad_silero.ipynb).
...
The loader returns `SileroVADForVoiceActivityDetection` through the shared task-specific factory.
