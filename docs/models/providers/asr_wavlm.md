---
description: Inference, data preparation, and training guide for the asr_wavlm integration.
---

# `asr_wavlm` model guide

`asr_wavlm` is a VoiceHub **automatic speech recognition**
integration. This page is generated from the model registry and its executable
data and training contracts, so the documented support stays aligned with code. [Open the `asr_wavlm` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_wavlm.ipynb).
...
The `asr_wavlm` contract is **integrated-raw**. Its
...
contract = get_asr_dataset_spec('asr_wavlm')
...
arguments = TrainingArguments(
    output_dir="runs/asr_wavlm-smoke",
    max_steps=1,
...
trainer.save_model("runs/asr_wavlm-smoke/final")
```
