...
WavLM uses the canonical model type `asr_wavlm` and is a
...
contracts, so the documented support stays aligned with code. [Open the `asr_wavlm` Colab notebook](https://colab.research.google.com/github/kadirnar/voicehub/blob/main/notebooks/models/asr_wavlm.ipynb).
...
WavLM waveform and CTC transcript records. Follow the [shared data workflow](../../guides/speech-data.md) for
...
The loader returns `WavLMForSpeechRecognition` through the shared task-specific factory.
