...
Реестр содержит **31 интеграцию TTS**, **9 провайдеров ASR** и **7 провайдеров
...
[каталог TTS](models/index.md), [матрицу обучения
TTS](models/training-support.md) и [матрицу поддержки
ASR/VAD](models/asr-vad-support.md).

Исходный код моделей входит в пакет VoiceHub. Необязательные extras
устанавливают зависимости выбранной среды выполнения, а веса checkpoint
загружаются по мере необходимости или указываются через локальные пути.
Лицензия Apache-2.0 распространяется на сам VoiceHub; для интегрированного
исходного кода, checkpoint, кодеков, наборов данных и сгенерированного аудио
могут действовать отдельные условия.
...
-   **Модели**
    ...
    Сравнивайте все 31 записи реестра: extras для установки, checkpoint по
    умолчанию, возможности, происхождение исходного кода и ограничения.

    [Каталог моделей](models/index.md)
...
-   **Notebook**
    ...
    Выполните полный рабочий процесс Dia: от базового инференса и проверки
    данных до обучения, экспорта и загрузки в новой среде выполнения.

    [Открыть руководство по Notebook](guides/notebook.md)
